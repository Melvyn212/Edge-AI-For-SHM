...
class Trainer(VarScopeObject):
    ...
    def __init__(self, model, model_vs=None, n_z=None,
                 ...
                 max_epoch=10, max_step=5000, batch_size=10,
    ...
    def fit(self, values,
            valid_portion=0.3, summary_dir=None,training_log="training_log.csv"):
        """
        ...
        with open(training_log, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Epoch', 'Step', 'Training Loss'])
            # training loop
            lr = self._initial_lr
            with TrainLoop(
