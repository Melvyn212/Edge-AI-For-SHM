...
def main():
    ...
    (x_train, _), (x_test, y_test) = \
        get_data(config.dataset, config.max_train_size, config.max_test_size, train_start=config.train_start,
                 test_start=config.test_start,prefix=prefix)
    ...
    with tf.compat.v1.variable_scope('model') as model_vs:
        ...
        with tf.Session().as_default():
            ...
            if config.max_epoch > 0:
                ...
                best_valid_metrics = trainer.fit(x_train,summary_dir=config.result_dir,training_log=training_log)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
            print("""
                Usage: python your_script.py <result_dir>
                where <result_dir> is the directory to save result files
                """)
            sys.exit(1)

        # Récupérer result_dir à partir des arguments de la ligne de commande
    config.result_dir = sys.argv[1]
    prefix=sys.argv[2]
    training_log=sys.argv[3]
