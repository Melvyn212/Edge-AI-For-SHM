...
# Based on https://github.com/ALLTERCO/shelly-script-examples/blob/main/tools/upload-script.sh
...
hash split 2>/dev/null || {
  echo >&2 "This script requires split, but it's not installed. Aborting"
  exit 1
}
...
list_scripts() {
    local SHELLY_HTTP_ENDPOINT="http://${SHELLY_IP}/rpc/Script.List"
    echo "Listing scripts "
    local RESULTS=$(curl -s "$SHELLY_HTTP_ENDPOINT")
    
    if [[ $RESULT == *"404"* ]]; then
        echo "Error calling $SHELLY_HTTP_ENDPOINT"
        exit 1
    fi
    local ERROR_CODE=$(echo $RESULT | jq ".code" -r)
...
upload_script() {
  ...
  split -b $CHUNK_SIZE --filter "cat -|put_chunk $ENDPOINT $1" $2
  echo ""
}
...
#process options
while getopts "s:i:f:hl" option; do
  #echo -n "args : $OPTARG"
  case $option in
  s)
    SHELLY_IP=$OPTARG
